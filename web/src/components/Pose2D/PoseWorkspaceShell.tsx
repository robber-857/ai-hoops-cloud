...
const modeConfig: Record<AnalysisMode, ModeConfig> = {
  ...
  dribbling: {
    ...
    guide: [
      ...
      'Template buttons are visual placeholders for now.',
    ],
    stats: [
...
export default function PoseWorkspaceShell({
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#07080b] text-white">
      ...
      <div className={cn('pointer-events-none absolute left-[-10rem] top-24 h-72 w-72 rounded-full blur-3xl', config.accentGlow)} />
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
          ...
          <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-3 gap-3 sm:w-fit">
                {config.stats.map((stat) => (
                  ...
                  >
                    ...
                    <div className={cn('mt-2 font-[var(--font-display)] text-lg font-semibold', config.accentText)}>
                      {stat.value}
                    </div>
                  </div>
                ))}
            ...
            <div className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr] xl:items-end">
              <div className="max-w-3xl">
                <div className={cn('font-[var(--font-display)] text-[0.72rem] font-semibold uppercase tracking-[0.34em]', config.accentText)}>
                  {config.eyebrow}
                </div>
              ...
              <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-4 backdrop-blur-xl sm:p-5">
                <div className="flex items-start gap-3">
                  <div className={cn('flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border text-white', config.accentBorder)}>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-6 text-white/55">
                      Users will later pick a template and see the matched example motion here before uploading their own clip.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.18fr_0.82fr]">
          <div className={cn('analysis-surface overflow-hidden rounded-[30px] border border-white/10 bg-gradient-to-br', config.accentSurface)}>
            <div className="analysis-grid relative min-h-[320px] overflow-hidden sm:min-h-[360px]">
              ...
              <div className="relative flex h-full flex-col justify-between p-5 sm:p-6 lg:p-7">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className={cn('inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[0.68rem] uppercase tracking-[0.28em] backdrop-blur-md', config.accentBorder, config.accentText)}>
          ...
          <div className="grid gap-6">
            <div className="analysis-surface rounded-[30px] border border-white/10 p-5 sm:p-6">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className={cn('rounded-full border px-3 py-1 text-[0.68rem] uppercase tracking-[0.24em]', config.accentBorder, config.accentText)}>
                  Placeholder
                </div>
              </div>
                ...
                {config.templates.map((template, index) => (
                  <button
                    ...
                    className={cn(
                      ...
                      index === 0
                        ? cn(config.accentButtonMuted, 'shadow-[0_12px_30px_rgba(15,23,42,0.25)]')
            ...
            <div className="analysis-surface rounded-[30px] border border-white/10 p-5 sm:p-6">
              ...
              <div className="mt-5 grid gap-3">
                {config.guide.map((item) => (
                  ...
                  >
                    <CheckCircle2 className={cn('mt-0.5 h-4 w-4 shrink-0', config.accentText)} />
        ...
        <section className="analysis-surface rounded-[32px] border border-white/10 p-4 sm:p-6">
          <div className="flex flex-col gap-4 border-b border-white/8 pb-5 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className={cn('inline-flex items-center gap-2 rounded-full border px-3 py-2 text-[0.72rem] uppercase tracking-[0.28em]', config.accentBorder, config.accentText)}>
