...
状态：图表视觉升级第一版已完成，后续重点是前端自动化测试、浏览器视觉回归和真实数据空态持续打磨。
...
- line chart 视觉升级已完成第一版：已使用多层背景、暗色科技网格、发光折线/节点、柔和扫描层、latest/peak 摘要和 hover/focus tooltip，让图表更像训练数据仪表盘。
...
1. 建设 Student 任务提交链路：任务详情跳转、提交入口、关联训练上传/报告、提交后 assignment 进度与状态更新、真实任务联动测试。
2. 补公告链路回归测试，覆盖 Admin announcement 列表/发布/归档/通知生成，以及 Coach class announcements 空列表/发布/读取/批量更新。
3. 补 Admin 用户管理权限、禁用历史数据保护、模板同步、任务监督和通知监督的后端自动化测试。
4. 继续打磨 Coach 聚合入口：为 `/coach/announcements`、`/coach/notifications` 补页面测试、批量已读和关联 class/task/report 的上下文跳转。
5. 继续打磨 Student 个人中心：独立消息中心、真实任务/公告页面测试、`Growth trends` 浏览器视觉回归和刷新策略。
6. 继续打磨 Admin class 成员添加：用户搜索/autocomplete、上传名单、API 层/页面测试。
7. 继续推进 Supabase signed upload、后端异步 AI 分析和模板示例视频审核发布流程。
