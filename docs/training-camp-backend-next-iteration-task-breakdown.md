...
- 创建/编辑用户时，用户名重复、邮箱重复、手机号重复应分别返回明确错误；多个字段重复时应能返回字段级明细：待完成。
...
- 优化 Admin announcement 的发布时间/过期时间输入：当前 `publish_at` / `expire_at` 仍让 Admin 手写 `ISO datetime`，非 IT 管理员很难判断格式；下一轮应改为日期选择 + 小时/分钟选择控件，交互可参考 Admin classes 页面日期选择，但需要额外选择具体时间，由前端组合成后端需要的时间值。
...
- Admin 公告表单时间输入体验优化：不能再让运营人员手写 `ISO datetime`，应使用日期 + 小时/分钟选择。
...
1. 优化 Admin 运营表单体验：announcement 起止时间改为日期 + 小时/分钟选择，由前端组合 `publish_at` / `expire_at`，不再要求手写 `ISO datetime`。
2. 优化 Admin 用户重复字段错误：注册或 Admin 创建/编辑用户时，`username`、`email`、`phone_number` 重复必须明确指出具体字段。
3. 升级 Student `Growth trends` line chart UI：在现有功能基础上做更有科技感的多层背景、发光折线、坐标网格和 tooltip。
4. 补公告链路回归测试，覆盖 Admin announcement 列表/发布/归档/通知生成，以及 Coach class announcements 空列表/发布/读取/批量更新。
5. 补 Admin 用户管理、模板同步、任务监督和通知监督的后端自动化测试。
6. 增强 Coach 聚合入口：补 `/coach/announcements`、`/coach/notifications`，让 Coach 能看到 Admin 全局/camp/角色公告。
7. 继续打磨 Student 个人中心：独立消息中心、任务详情跳转、真实任务/公告页面测试和刷新策略。
8. 继续打磨 Admin class 成员添加：用户搜索/autocomplete、上传名单、API 层/页面测试。
9. 继续推进 Supabase signed upload、后端异步 AI 分析和模板示例视频审核发布流程。
